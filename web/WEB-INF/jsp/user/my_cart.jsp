	...
	<script type="text/javascript">
		$(function(){
			...
			check.click(function(){
				...
				if ($(this).is(":checked")) {
					//改变结算按钮样式
					$(".settlement").css("background", "#000000");
			...
			$(".settlement").click(function(){
				form.attr("action", "cart?method=settlement");
				form.submit();
			});
			...
			$("#c_con_down_d1 a").click(function(){
				...
				form.attr("action", "cart?method=deleteMore");
				//弹框单击确定事件
			...
			$(".td6 a").click(function() {
				...
				//弹框单击确定事件
				$("#confirm").click(function(){
					form.attr("action", "cart?method=deleteOne&cid=" + cartid);
			...
			$(".minus").click(function() {
				...
				if(num > 1) {
					...
					$.ajax({
						type:"post",
						url:"cart",
						data:{cid:cid, option:"minus", method:"updateQuantity"},
			...
			$(".plus").click(function() {
				...
				if(num < inventory) {
					...
					$.ajax({
						type:"post",
						url:"cart",
						data:{cid:cid, option:"plus", method:"updateQuantity"},
						success:function(data) {
							if(data == "yes") {
								//页面强制刷新,用于显示或更改购物中心的商品记录数
								window.location.reload();
